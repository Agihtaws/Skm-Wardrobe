...
export default function CategoriesClient({ initialCategories }: Props) {
  ...
  const renderNode = (cat: Category, depth = 0) => {
    ...
    return (
      <div key={cat.id}>
        <div className={`flex items-center gap-2 px-3 py-2.5 rounded-lg group hover:bg-gray-50 ${depth > 0 ? "ml-4 sm:ml-6" : ""}`}>
          {/* Expand toggle */}
          {children.length > 0 ? (
          ...
          {(cat as any).image_url ? (
            <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg overflow-hidden flex-shrink-0 border border-gray-100">
          ...
          {/* Slug — hide on very small screens */}
          <span className="hidden sm:block text-xs text-gray-400 font-mono mr-2 shrink-0">/{cat.slug}</span>

          {/* Action buttons — always visible on mobile, hover-only on desktop */}
          <div className="flex items-center gap-0.5 sm:gap-1 sm:opacity-0 sm:group-hover:opacity-100 sm:transition-opacity">
            <button
  ...
  return (
    <div className="p-3 sm:p-5 lg:p-6 max-w-4xl">
      ...
      <div className="flex items-center justify-between mb-5 sm:mb-6">
        ...
        <button
          onClick={openAdd}
          className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-pink-600 hover:bg-pink-700 text-white text-sm font-medium rounded-lg transition-colors"
        >
          <Plus size={16} />
      ...
      {/* ── Add / Edit form ── */}
      {showForm && (
        <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-5 mb-5 sm:mb-6 shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-3 sm:gap-4">
                {form.image_url ? (
                  <div className="relative w-20 h-24 sm:w-24 sm:h-28 rounded-xl overflow-hidden border border-gray-200 bg-gray-50 flex-shrink-0">
                  ...
                  <div className="w-20 h-24 sm:w-24 sm:h-28 rounded-xl border-2 border-dashed border-gray-200 bg-gray-50 flex items-center justify-center flex-shrink-0">
                ...
                <div>
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-3 sm:px-4 py-2 border border-gray-200 text-sm text-gray-600 rounded-lg hover:bg-gray-50 disabled:opacity-60"
                  >
                    {imgUploading ? <Loader2 size={14} className="animate-spin" /> : <ImageIcon size={14} />}
                    {imgUploading ? "Uploading…" : "Upload Image"}
                  </button>
                  <p className="text-xs text-gray-400 mt-1.5">PNG, JPG up to 3MB.</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col-reverse sm:flex-row gap-2 sm:gap-3 sm:justify-end pt-2">
              ...
              <button
                type="submit"
                disabled={loading}
                className="flex items-center justify-center gap-2 px-5 py-2 text-sm bg-pink-600 hover:bg-pink-700 text-white font-medium rounded-lg disabled:opacity-60 min-w-[130px]"
              >
                {loading
