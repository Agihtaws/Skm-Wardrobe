  ...
  return (
    <div className="p-3 sm:p-5 lg:p-6 max-w-4xl">
      ...
      <div className="flex items-start gap-3 mb-5 sm:mb-6">
        <Link href="/admin/orders" className="text-gray-400 hover:text-gray-600 mt-1 shrink-0">
          <ChevronLeft size={20} />
        </Link>
              ...
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>
                  {["Product", "Price", "Qty", "Total"].map((h) => (
                    <th key={h} className="px-4 sm:px-5 py-3 text-left text-xs font-semibold text-gray-500 uppercase whitespace-nowrap">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="divide-y divide-gray-50">
                {items.map((item: any) => (
                  <tr key={item.id}>
                    ...
                    <td className="px-4 sm:px-5 py-3 font-semibold whitespace-nowrap text-xs sm:text-sm">
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-4 sm:p-5 space-y-4">
          ...
          {order.shiprocket_order_id && (
            <div className="bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 space-y-1">
              ...
              <p className="text-xs text-blue-600 font-mono break-all">SR ID: {order.shiprocket_order_id}</p>
              {order.shiprocket_shipment_id && (
                <p className="text-xs text-blue-600 font-mono break-all">Shipment: {order.shiprocket_shipment_id}</p>
              )}
              {order.courier_name && (
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-500 mb-1">
              ...
              <span className="font-normal text-gray-400 ml-1">(use only if needed)</span>
            </label>
            <select
              value={status}
